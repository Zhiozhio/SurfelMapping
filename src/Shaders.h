...
{
    std::string dir = Config::shaderDir();
    std::shared_ptr<Shader> program = std::make_shared<Shader>();

    program->AddShaderFromFile(pangolin::GlSlVertexShader, dir + "/" + vertex_shader_file, {}, {dir});
    program->AddShaderFromFile(pangolin::GlSlGeometryShader, dir + "/" + geometry_shader_file, {}, {dir});
    if (!program->Link())
        printf("[Warning] shader link have some problem!\n");
...
static inline std::shared_ptr<Shader> loadProgramFromFile(const std::string& vertex_shader_file, const std::string& fragment_shader_file)
{
    ...
    program->AddShaderFromFile(pangolin::GlSlVertexShader, dir + "/" + vertex_shader_file, {}, {dir});
    program->AddShaderFromFile(pangolin::GlSlFragmentShader, dir + "/" + fragment_shader_file, {}, {dir});
    if (!program->Link())
        printf("[Warning] shader link have some problem!\n");
...
static inline std::shared_ptr<Shader> loadProgramFromFile(const std::string& vertex_shader_file, const std::string& geometry_shader_file, const std::string& fragment_shader_file)
{
    std::string dir = Config::shaderDir();
    std::shared_ptr<Shader> program = std::make_shared<Shader>();
