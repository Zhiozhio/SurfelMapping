...
class SPADEGenerator(BaseNetwork):
    ...
    def __init__(self, opt):
        ...
        self.sw, self.sh = self.compute_latent_vector_size(opt)  # semantic_nc =184
    ...
    def compute_latent_vector_size(self, opt):
        ...
        sw = opt.crop_size // (2**num_up_layers)  # opt.crop_size = 256
        sh = round(sw / opt.aspect_ratio)

        return sw, sh

    def forward(self, input, z=None):
        seg = input  # seg is a tensor with size[1,184,256,256]
        ...
        if self.opt.use_vae:
            ...
            x = F.interpolate(seg, size=(self.sh, self.sw)) #[1,184,8,8]
